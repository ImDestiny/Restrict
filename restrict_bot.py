        ...
        if user.get("session"):
            ...
            if user_tasks:
                ...
                for t_id, info in user_tasks.items():
                    ...
                    task_details.append(
                        f"      └ 🏃 **{src}** → **{dst}**\n"
                        f"        {generate_bar(percent)} (`{curr}/{tot}`)\n"
                        f"        ⏳ **ETA:** `{eta_str}`"
                    )
                tasks_str = "\n" + "\n".join(task_details)
...
async def process_links_logic(client: Client, message: Message, text: str, dest_chat_id=None, dest_thread_id=None, delay=3, acc_user_id=None, task_uuid=None, is_restricted=False):
    ...
    if "https://t.me/" in text:
        ...
        try:
            ...
            # --- 🔑 LOGIN / SESSION START (Corrected - No Duplicates) ---
            if LOGIN_SYSTEM:
            ...
            try:
                ...
                source_title = source_chat.title or source_chat.first_name or "Private Chat"
            except:
                pass

            # Update ACTIVE_PROCESSES tracking
            if user_id in ACTIVE_PROCESSES and task_uuid in ACTIVE_PROCESSES[user_id]:
                ACTIVE_PROCESSES[user_id][task_uuid].update({
                    "source_title": source_title,
                    "dest_title": dest_chat_id, 
                    "total": total_count,
                    "current": 0
                })
            ...
            for index, msgid in enumerate(range(fromID, toID+1), start=1):
                ...
                try:
                    ...
                    # This function handles the full Download + Upload
                ...
                # 2. WAIT (The sequence you requested: Process -> Wait -> Next)
                if index < total_count: # Don't wait after the very last message
                    await asyncio.sleep(delay)

                # 3. DASHBOARD UPDATE (Every 10 messages or 30 seconds)
                if (index % 10 == 0):
                    elapsed = time.time() - start_time
            ...
            final_text = f"**Batch {'Cancelled' if was_cancelled else 'Completed'}!** {'🛑' if was_cancelled else '✨'}\n\n**Total:** `{total_count}`\n**Saved:** `{success_count}`\n**Failed:** `{failed_count}`"
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int, index: int, total_count: int, status_message: Message, dest_chat_id, dest_thread_id, delay, user_id, task_uuid=None, is_restricted=False):
    ...
    up_task = None
    try:
        # 4. DOWNLOAD (Background Status)
        down_task = asyncio.create_task(downstatus(client, status_message, message.chat.id, index, total_count))
        
        file_path = await acc.download_media(msg, file_name=str(task_folder_path / "file"), progress=progress, progress_args=[status_message, "down", task_uuid])
        
        # Kill download status as soon as file is on disk
        if down_task: down_task.cancel()
        ...
        # 5. SPLIT & UPLOAD LARGE FILES
        if file_size > split_limit:
            await status_message.edit_text(f"✂️ **Large file detected ({_pretty_bytes(file_size)})**\nSplitting into parts...")
            parts = await split_file_python(file_path, chunk_size=1900*1024*1024)
            
            # Start background upload status
        ...
        # 6. UPLOAD NORMAL FILES
        # FIX: Changed from 'await' to 'create_task' so it doesn't block the upload
        up_task = asyncio.create_task(upstatus(client, status_message, message.chat.id, index, total_count))
        
        uploader = client if file_size < 2000 * 1024 * 1024 else acc
        
        async with USER_UPLOAD_LOCKS[user_id]:
            async with UPLOAD_SEMAPHORE:
                if "Document" == msg_type: await uploader.send_document(dest_chat_id, file_path, caption=caption, message_thread_id=dest_thread_id, progress=progress, progress_args=[status_message,"up", task_uuid])
                elif "Video" == msg_type: await uploader.send_video(dest_chat_id, file_path, caption=caption, message_thread_id=dest_thread_id, progress=progress, progress_args=[status_message,"up", task_uuid])
                elif "Audio" == msg_type: await uploader.send_audio(dest_chat_id, file_path, caption=caption, message_thread_id=dest_thread_id, progress=progress, progress_args=[status_message,"up", task_uuid])
                elif "Photo" == msg_type: await uploader.send_photo(dest_chat_id, file_path, caption=caption, message_thread_id=dest_thread_id)
                elif "Voice" == msg_type: await uploader.send_voice(dest_chat_id, file_path, caption=caption, message_thread_id=dest_thread_id)
        ...
        # CRITICAL: Always kill background status tasks so they don't leak memory
