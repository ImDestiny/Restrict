...
class Database:
    ...
    async def save_task(self, task_data):
        """Saves a task so it can be resumed after restart."""
        return await self.db.tasks.update_one(
            {"task_uuid": task_data["task_uuid"]}, 
            {"$set": task_data}, 
            upsert=True
        )

    async def update_task_progress(self, task_uuid, current_id):
        """Saves the current message ID checkpoint."""
        await self.db.tasks.update_one(
            {"task_uuid": task_uuid}, 
            {"$set": {"current_msg_id": current_id}}
        )

    async def delete_task(self, task_uuid):
        """Removes the task once completed."""
        await self.db.tasks.delete_one({"task_uuid": task_uuid})

    async def get_all_unfinished_tasks(self):
        """Fetches tasks to resume on startup."""
...
async def logout(client, message):
    ...
    # 3. Clean up Local Database & Remove Pending Tasks
    ...
    # NEW: Remove unfinished tasks from MongoDB so they don't resume
    await db.db.tasks.delete_many({"user_id": user_id})

    # NEW: Signal immediate cancellation for any running tasks
    if user_id in ACTIVE_PROCESSES:
        for t_uuid in list(ACTIVE_PROCESSES[user_id].keys()):
            CANCEL_FLAGS[t_uuid] = True
    
    await message.reply("**Logout Complete** ♦\n\n✅ Your session and all pending/active tasks have been cleared.")
...
async def process_links_logic(client: Client, message: Message, text: str, dest_chat_id=None, dest_thread_id=None, delay=3, acc_user_id=None, task_uuid=None, is_restricted=False):
    ...
    # Define safe targets for resume logic
    target_chat = message.chat.id if message else user_id
    reply_id = message.id if message else None

    if dest_chat_id is None: dest_chat_id = target_chat
    if dest_thread_id is None: dest_thread_id = getattr(message, 'message_thread_id', None)
    ...
    if "https://t.me/" in text:
        ...
        try:
            ...
            status_message = await client.send_message(
                target_chat, # Use the safe variable
                ...
                f"**Failed/Skipped:** `0`\n"
                f"**ETA:** `...`",
                reply_to_message_id=reply_id # Use the safe variable
            )
            ...
            if LOGIN_SYSTEM == True:
                ...
                if user_data is None:
                    await client.send_message(target_chat, "**/login First.**"); raise ValueError("Not logged in") # Use target_chat
                api_id = await db.get_api_id(user_id)
                api_hash = await db.get_api_hash(user_id)
                try:
                    ...
                    await client.send_message(target_chat, f"**Login Failed:** `{e}`"); raise ValueError("Login failed") # Added await and target_chat
                    
            else:
                if GlobalUserSession is None: await client.send_message(target_chat, "**Error:** Global Session Missing.")
                acc = GlobalUserSession

            # 1. Save Task to DB before starting loop
            await db.save_task({
                "task_uuid": task_uuid, "user_id": user_id, "link": text,
                "from_id": fromID, "to_id": toID, "current_msg_id": fromID,
                "dest_chat_id": dest_chat_id, "dest_thread_id": dest_thread_id,
                "delay": delay, "is_restricted": is_restricted
            })
            ...
            for index, msgid in enumerate(range(fromID, toID+1), start=1):
                ...
                try:
                    ...
                    if e.value > 120:
                        # Use target_chat for safety in case of resume
                        target_chat = message.chat.id if message else user_id
                        await client.send_message(target_chat, f"🚨 **FloodWait Too Long ({e.value}s). Task Aborted.**")
                        return
                ...
                # 2. Update Progress Checkpoint in DB after every message
                await db.update_task_progress(task_uuid, msgid)

                # --- 3. VISUAL STATUS UPDATE (Frequency Restored to 20 msgs / 60 sec) ---
                current_time = time.time()
                if (index % 20 == 0) or (index == total_count) or ((current_time - last_update_time) > 60):
                    ...
                    if elapsed > 0:
                        ...
                        eta_seconds = (total_count - index) / items_per_second if items_per_second > 0 else 0
                    ...
                    p_bar = generate_bar(percent, length=12)

                    try:
                        await status_message.edit_text(
                            f"{status_text_header}\n"
                            f"**Progress:** {p_bar} `{percent:.1f}%`\n"
                ...
                # Sleep based on restriction
                if is_success:
                    if is_restricted:
                        await asyncio.sleep(delay)
                    else:
                        time_taken = time.time() - step_start_time
                        await asyncio.sleep(max(0, delay - time_taken))
                else:
            ...
            # --- ENHANCED TASK SUMMARY REPORT ---
            duration = get_readable_time(int(time.time() - start_time))
            status_icon = "🛑" if ('was_cancelled' in locals() and was_cancelled) else "✅"
            status_label = "Cancelled" if status_icon == "🛑" else "Completed"

            final_text = (
                f"{status_icon} **Batch {status_label}!**\n\n"
                f"👤 **User:** {user_mention}\n"
                f"🆔 **Task ID:** `{task_uuid[:8]}`\n"
                f"⏱ **Time Taken:** `{duration}`\n"
                f"📊 **Statistics:**\n"
                f"├ 📥 **Total Requested:** `{total_count}`\n"
                f"├ ✅ **Successful:** `{success_count}`\n"
                f"└ ❌ **Failed/Skipped:** `{failed_count}`\n\n"
                f"✨ *Thank you for using the service!*"
            )
            
            try: 
                await client.send_message(target_chat, final_text, reply_to_message_id=reply_id)
                # Cleanup: Delete task from DB upon final completion
                await db.delete_task(task_uuid)
            except: 
                pass
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int, index: int, total_count: int, status_message: Message, dest_chat_id, dest_thread_id, delay, user_id, task_uuid=None):
    ...
    # 2. DOWNLOAD & UPLOAD
    # Use status_message.chat.id instead, it's safer!
    asyncio.create_task(downstatus(client, status_message, status_message.chat.id, index, total_count))
    ...
    try: 
        ...
        asyncio.create_task(upstatus(client, status_message, status_message.chat.id, index, total_count))
...
async def resume_all_tasks(client: Client):
    """Checks MongoDB for unfinished tasks and restarts them."""
    cursor = await db.get_all_unfinished_tasks()
    async for task in cursor:
        uid = task['user_id']
        curr_id = task['current_msg_id']
        to_id = task['to_id']
        
        remaining = to_id - curr_id
        if remaining <= 0:
            await db.delete_task(task['task_uuid'])
            continue

        # Enhanced Notification for User Transparency
        try:
            await client.send_message(
                uid, 
                f"🔄 **System Recovery Active**\n\n"
                f"**Task ID:** `{task['task_uuid'][:8]}`\n"
                f"**Resuming From:** `{curr_id}`\n"
                f"**Target ID:** `{to_id}`\n"
                f"**Messages Left:** `{remaining}`\n\n"
                f"⏳ *Please wait while I re-initialize the connection...*"
            )
        except Exception as e:
            print(f"Failed to notify user {uid} of resume: {e}")
           
        # Construct resume link (e.g., 251-500)
        base_link = task['link'].split('-')[0].rsplit('/', 1)[0]
        resume_link = f"{base_link}/{curr_id}-{to_id}"

        asyncio.create_task(process_links_logic(
            client=client,
            message=None, # Context is gone, logic uses acc_user_id
            text=resume_link,
...
async def main():
    ...
    print("Bot Started. Resuming unfinished tasks...")
    asyncio.create_task(resume_all_tasks(app))
