...
def generate_bar(percent: float, length: int = 12) -> str:
    # Bzex Style: ⬢ for filled, ⬡ for empty
...
async def get_user_status_text(user_id):
    """Generates the Styled multi-task status for a specific user."""
    user_tasks = ACTIVE_PROCESSES.get(user_id, {})
    if not user_tasks:
        return "No active tasks."

    lines = [f"**Bot Of Restrict Content**\n"]
    
    for i, (tid, info) in enumerate(user_tasks.items(), 1):
        # Determine if we are downloading or uploading for this specific task
        # We check the PROGRESS dictionary using the message ID associated with the task
        # Note: This assumes you store the status_msg_id in ACTIVE_PROCESSES
        msg_id = info.get("status_msg_id")
        
        # Try to find download progress, then upload progress
        rec = PROGRESS.get(f"{msg_id}:down") or PROGRESS.get(f"{msg_id}:up")
        status_type = "Download" if PROGRESS.get(f"{msg_id}:down") else "Upload"
        
        if not rec:
            lines.append(f"{i}. `{info.get('item')[:40]}...`\nPreparing...\n")
            continue

        p = rec.get('percent', 0)
        curr = _pretty_bytes(rec.get('current', 0))
        tot = _pretty_bytes(rec.get('total', 0))
        spd = _pretty_bytes(rec.get('speed', 0))
        eta = get_readable_time(int(rec.get('eta', 0)) if rec.get('eta') else 0)

        lines.append(
            f"{i}. {info.get('item')[:60]}\n\n"
            f"{info.get('user')} ({user_id})\n"
            f"╭{generate_bar(p)} {p:.2f}%\n"
            f"┊Processed » {curr} of {tot}\n"
            f"┊Status » {status_type}\n"
            f"┊Speed » {spd}/s\n"
            f"┊Time » {eta}\n"
            f"┊Engine » Pyrofork v2.3.45.\n"
            f"┊In Mode » #TgMedia\n"
            f"┊Out Mode » #Restricted\n"
            f"╰Stop » `/cancel`"
        )
...
async def start_task_final(client: Client, message_context: Message, task_data: dict, delay: int, user_id: int):
    ...
    ACTIVE_PROCESSES[user_id][task_uuid] = {
        ...
        "started": time.time(),
        "status_msg_id": None  # <--- Added this placeholder
    }
