...
async def process_links_logic(client: Client, message: Message, text: str, dest_chat_id=None, dest_thread_id=None, dest_title="Direct Message", delay=3, acc_user_id=None, task_uuid=None, is_restricted=False):
    ...
    if "https://t.me/" in text:
        ...
        try:
            ...
            # --- NEW STYLED COMPLETION MESSAGE ---
            # 1. Calculate Duration
            duration = time.time() - start_time
            time_taken_str = get_readable_time(int(duration))
            
            # 2. Determine Header (Cancelled vs Completed)
            if 'was_cancelled' in locals() and was_cancelled:
                header = f"🛑 **Batch Cancelled!** {user_mention} ✨"
            else:
                header = f"✅ **Batch Completed!** {user_mention} ✨"

            # 3. Build the Statistic Message
            final_text = (
                f"{header}\n"
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int, index: int, total_count: int, status_message: Message, dest_chat_id, dest_thread_id, delay, user_id, task_uuid=None):
    ...
    # 1. FAST FORWARD (Copy)
    if not getattr(msg, "has_protected_content", False) and not getattr(msg.chat, "has_protected_content", False):
        try:
    ...
    try: 
        for attempt in range(3):
            ...
            try:
                ...
                if file_size > split_limit:
                    ...
                    # Explicitly cancel the download status. This kills the sleep loop immediately.
        ...
        # --- CRITICAL: INSTANT STATUS SWITCH ---
        # Cancellation here ensures we don't wait for the 20s timer to finish
        if down_task and not down_task.done(): down_task.cancel()
