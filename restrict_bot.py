...
async def process_links_logic(client: Client, message: Message, text: str, dest_chat_id=None, dest_thread_id=None, dest_title="Direct Message", delay=3, acc_user_id=None, task_uuid=None, is_restricted=False):
    ...
    ACTIVE_PROCESSES[user_id][task_uuid] = {
        "user": user_mention, 
        "dest_title_name": dest_title, # For /status and /botstats
        "item": text[:50]+"...", 
        "started": time.time()
    }
    ...
    if "https://t.me/" in text:
        ...
        try:
            ...
            for index, msgid in enumerate(range(fromID, toID+1), start=1):
                ...
                if index < total_count: await asyncio.sleep(delay)

                # --- NEW UPDATE LOGIC: Every 20 messages OR every 60 seconds ---
                ...
                if (index % 20 == 0) or (current_now - last_update_time >= 60) or index == total_count:
                    ...
                    try:
                        ...
                        # Just skip this update if we are rate limited to avoid stopping the whole task
                        print(f"Skipping status update due to floodwait: {e.value}")
                        pass
                    except Exception:
                        pass                       
