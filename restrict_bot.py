            ...
            for index, msgid in enumerate(range(fromID, toID+1), start=1):
                ...
                try:
                    ...
                    is_success = await handle_private(
                        ...
                        filter_thread_id=filter_thread_id # <--- PASSING THE FILTER ID
                    )
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int, index: int, total_count: int, status_message: Message, dest_chat_id, dest_thread_id, delay, user_id, task_uuid=None, is_restricted=False, header_text="", filter_thread_id=None):
    msg = None
    try:
    ...
    # --- TOPIC FILTER CHECK ---
    # If a filter is active, skip messages that don't belong to this topic
