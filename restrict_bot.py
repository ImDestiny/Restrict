...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int, index: int, total_count: int, status_message: Message, dest_chat_id, dest_thread_id, delay, user_id, task_uuid=None, is_restricted=False):
    ...
    # This prevents the UI from getting stuck on the previous file's 100% status
    try:
        if status_message:
    ...
    # Trigger the "Downloading..." status update immediately
    ...
    try: 
        ...
        # 4. START UPLOAD
        # Clear specific upload progress key before starting
        if f"{status_message.id}:up" in PROGRESS: del PROGRESS[f"{status_message.id}:up"]
