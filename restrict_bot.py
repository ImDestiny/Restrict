...
async def process_links_logic(client: Client, message: Message, text: str, dest_chat_id=None, dest_thread_id=None, dest_title="Direct Message", delay=3, acc_user_id=None, task_uuid=None, is_restricted=False):
    ...
    if "https://t.me/" in text:
        ...
        total_count = 0
        status_message = None
        filter_thread_id = None # Initialize variable
        ...
        try:
            ...
            # --- RESTORED TOPIC DETECTION LOGIC ---
            ...
            # Use filter_thread_id to adjust starting point if needed
            if filter_thread_id and fromID < filter_thread_id:
                # This logic assumes the topic ID acts as a lower bound for msg IDs in some contexts,
                # but mainly we need it for the Header display.
                # In standard Telegram, msg IDs are global in a chat, so we just keep the IDs.
                pass 
            ...
            # --- STATUS MESSAGE SETUP (Restored Header Logic) ---
            ...
            # Pass just the "Filter: ..." part to handle_private for the inner status
            ...
            for index, msgid in enumerate(range(fromID, toID+1), start=1):
                ...
                try:
                    ...
                    is_success = await handle_private(
                        ...
                        user_id, task_uuid, is_restricted=is_restricted, 
                        header_text=inner_header # <--- Passing the Filter Text
                    )
                ...
                if index < total_count:
                    if is_success:
                        ...
                        # Failed/Deleted/Missing? Skip delay instantly (0.05s)
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int, index: int, total_count: int, status_message: Message, dest_chat_id, dest_thread_id, delay, user_id, task_uuid=None, is_restricted=False, header_text=""):
    msg = None
    try:
    ...
    # --- FIX: CLEAR PROGRESS DICT FOR THIS MSG_ID ---
    try:
        if status_message:
    ...
    if not is_restricted and not getattr(msg, "has_protected_content", False) and not getattr(msg.chat, "has_protected_content", False):
        try:
            ...
            if e.value > 300: raise e # Propagate to main loop to cancel
    ...
    # Passed header_text here
    down_task = asyncio.create_task(downstatus(client, status_message, message.chat.id, index, total_count, header_text))
    file_path = None
    ph_path = None
        ...
        for attempt in range(3):
            ...
            try:
                ...
                if file_size > split_limit:
                    ...
                    up_task = asyncio.create_task(upstatus(client, status_message, message.chat.id, index, total_count, header_text))
